...
helpers do
  ...
  def commit(name, caption)
    %Q{<div class="commit"><a href="https://github.com/SachaG/Microscope/commit/chapter#{name}" target="_blank">Commit #{name}: #{caption}</a><a href="http://meteor-book-chapter#{name}.meteor.com" target="_blank" class="live-instance">Launch live instance #{name}</a></div>}
  end  
